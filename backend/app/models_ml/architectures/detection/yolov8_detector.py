...
class Detection:
    def __init__(self, bbox: tuple[int, int, int, int], class_label: str, confidence: float, class_id: int = 0):
        self.bbox = bbox
...
class YOLODetector:
    ...
    def __init__(self, model_path: Path | str | None = None, class_names: list[str] | None = None, conf_threshold: float = 0.25):
    ...
    def detect(self, image: np.ndarray) -> list[Detection]:
        ...
        for result in results:
            ...
            for i in range(len(boxes)):
                ...
                detections.append(Detection(bbox=(x1, y1, x2, y2), class_label=label, confidence=conf, class_id=cls_id))
    ...
    @classmethod
    def train(cls, data_yaml: str, model_size: str = "n", epochs: int = 100, imgsz: int = 640, device: str = "cpu"):
        try:
            from ultralytics import YOLO
            model = YOLO(f"yolov8{model_size}.pt")
            results = model.train(data=data_yaml, epochs=epochs, imgsz=imgsz, device=device, project="runs/detect", name="agriai")
            return results
        except ImportError:
