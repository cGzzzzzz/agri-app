...
def send_message(payload: ChatRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    conversation, context = ChatService(db).send(current_user, payload.message, payload.farm_id, payload.crop_id, payload.input_type)
    context_dict = {
        "user": context.user,
        "farm": context.farm,
        "crop": context.crop,
    ...
    } if hasattr(context, "user") else context
    data = ChatResponse(response=conversation.response, conversation=ChatRead.model_validate(conversation), context=context_dict)
...
def history(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return ok([ChatRead.model_validate(item) for item in conversations], "Conversation history loaded")
