...
class VisionPipeline:
    def __init__(
        self,
        crop_predictor: CropPredictor | None = None,
        ...
        self.crop_predictor = crop_predictor or ExplainableCropPredictor()
        self.disease_predictor = disease_predictor or ExplainableDiseasePredictor()
        self.severity_predictor = severity_predictor or ExplainableSeverityPredictor()
    ...
    def analyze(
        ...
        trace.append({"step": "image_preprocessing", "status": "completed", "data": asdict(features)})
        ...
        trace.append({"step": "model_registry", "status": "completed", "data": [asdict(card) for card in model_cards]})
            ...
            try:
                ...
                if loaded:
                    gradcam_b64 = self.gradcam.generate_heatmap(
                        loaded.session, processed.tensor, input_name=loaded.session.get_inputs()[0].name
                    )
            except Exception:
                pass
            trace.append({
